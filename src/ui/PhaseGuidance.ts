...
export class PhaseGuidance {
  ...
  getPhaseToast(phase: string): string | null {
    const tips: Record<string, string> = {
      ...
      play: 'Mobilize (🏭), move units, and attack — then End Turn. Press T in battle preview for tactical command.',
  ...
  getFirstTurnTip(turnNumber: number, phase: string): FirstTurnTip | null {
    ...
    const tips: Record<string, string> = {
      ...
      play: 'First turn: mobilize at your capital or factory, drag units to a highlighted neighbor, then attack if the preview looks favorable. Try Play Tactical (T) on contested fights.',
